...
- 当检测到新的一天时，自动执行签到
- **新增**: 在 `perform_daily_checkin()` 中添加1秒延迟，避免向多个群组快速发送消息时遇到速率限制
...
- 每日签到时在发送给不同群组之间添加1秒延迟，避免触发Telegram速率限制

## 总结 (Summary)

✅ **问题1**: 每日签到现在会在monitor启动时和每日首次收到消息时自动执行，并向**所有**监控的群组发送签到消息（群组间延迟1秒以避免速率限制）
